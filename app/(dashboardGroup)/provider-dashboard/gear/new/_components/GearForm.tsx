...
const GearForm = ({
  ...
  const [images, setImages] = useState<FieldRow[]>(
    defaultValues?.images?.length
      ? defaultValues.images.map(() => ({ id: crypto.randomUUID() }))
      : [{ id: crypto.randomUUID() }],
  );
  const [specs, setSpecs] = useState<FieldRow[]>(
    defaultValues?.specifications && Object.keys(defaultValues.specifications).length
      ? Object.keys(defaultValues.specifications).map(() => ({ id: crypto.randomUUID() }))
      : [{ id: crypto.randomUUID() }],
  );
  ...
  const specEntries = defaultValues?.specifications
    ? Object.entries(defaultValues.specifications)
    : [];
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input id="name" name="name" placeholder="e.g. Mountain Bike" defaultValue={defaultValues?.name} required />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input id="brand" name="brand" placeholder="e.g. Trek" defaultValue={defaultValues?.brand ?? ""} />
            </div>
          </div>
          ...
          {images.map((row, index) => (
            <div key={row.id} className="flex items-center gap-2">
              <Input
                ...
                type="url"
                defaultValue={defaultValues?.images?.[index] ?? ""}
              />
              {images.length > 1 && (
                <Button
                  type="button"
          ...
          {specs.map((row, index) => (
            <div key={row.id} className="flex items-center gap-2">
              <Input
                ...
                className="flex-1"
                defaultValue={specEntries[index]?.[0] ?? ""}
              />
              <Input
                name="specValue"
                placeholder="Value (e.g. 12kg)"
                className="flex-1"
                defaultValue={specEntries[index]?.[1] ?? ""}
              />
              {specs.length > 1 && (
                <Button
                  type="button"
